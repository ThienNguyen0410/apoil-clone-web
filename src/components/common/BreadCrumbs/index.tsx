...
export default function BreadCrumbBar({name, hasTabs, tabs, icon} : BreadCrumbProps)  {
  ...
  return (
    <div className="breadcrumb-box"> 
        ...
        {!hasTabs ? (
            <div className="breadcrumb-no-tabs">
                {icon}
                <span>{name}</span>
            </div>
        ) : (
            ...
            items={[
            {
                title: <span className="home-tab">{icon} {t(name)}</span>,
            },
            ...(tabs?.map((tab) => ({
                title: <span>{t(tab)}</span>,
            })) ?? []),
        ]}
            />
        )}
